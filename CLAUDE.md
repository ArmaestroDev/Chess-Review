...
**Desktop scaling — single source of truth (`shared/utils/layout.ts`).** The desktop UI is a fixed-pixel design; it only looked right at one effective viewport size (browser 100% on the author's display) and broke at other browser-zoom levels. `layout.ts` is now the **only** place scaling is decided: it owns the layout constants (`LAYOUT`), the reference design size, the UI-scale clamp, the board-sizing formula (`computeBoardSize` / `useBoardSize`), and the `--lyt-*` / `--ui-scale` CSS vars. `AppLayout` wraps the desktop header+routes in a uniformly `zoom`-scaled "design canvas" (`useUiCanvas` + `canvasStyle`) sized to exactly cover the viewport, so the known-good layout fits at any browser zoom; modals stay outside the zoom; mobile is never scaled (the wrapper is `display:contents`). `ReviewDesktop`/`PuzzleHubDesktop` get their board size from `useBoardSize`, and the `.pz-hub-grid` / ReviewDesktop grid templates + `max-w` read the `--lyt-*` vars — so the column widths and the board formula can no longer drift (the old hazard). **Any scaling change goes here; never hardcode column/board px or a new `window` resize listener in a page.**
