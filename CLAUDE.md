...
**Sounds.** `utils/sounds.ts` synthesizes move/capture/check/checkmate/castle/error chirps with WebAudio — no audio files are bundled. Only the review page plays them, and only after a move is fully analyzed.
...
- **Bundled catalog.** `data/catalog/{tier}.json` + `data/daily.json` + `data/themes.json` are imported eagerly by `api/catalog.ts` (still inside the lazy chunk). Built by `scripts/build-catalog.ts` from the Lichess CSV; do **not** hand-edit them.
...
- **Solver punisher.** When the user plays a wrong move, `usePuzzleSession` emits `analyzeMove` so the engine's best refutation animates on the board. This piggybacks on the same socket as the review page — there is no puzzle-specific channel.
